...
## ファイル内検索（component/viewer/search.rs）
- `/` の入力中は 1 文字ごとにマッチを引き直す（ライブプレビュー）。**キー処理が文書全体の走査になる唯一の経路**なので、そのままだと 2 万行で 1 打鍵 7ms 近くかかっていた（`cargo perf` の `search-type`）。2 段で削っている:
  - **照合はバイト列のまま**（`line_matches`）。畳み込みが ASCII 限定なので非 ASCII のバイトは素のまま比べればよく、UTF-8 は一致が必ず char 境界から始まるので char 列で比べた結果と同じになる。桁（char インデックス）は一致した位置でだけ数える。行ごとに `Vec<char>` を 2 本確保し直していたのをやめ、先頭バイトで読み飛ばす
...
- **Enter は自動インデント、Tab はソフトタブ**（`component/editor/indent.rs`、word.rs と同じく純関数）。規則は「前の行のインデントを引き継ぐ」「開き括弧 `{ ( [` の直後で 1 段下げ、直後が対応する閉じ括弧なら 3 行に開く」「割った位置の前後の空白は捨てる（元の行の末尾に空白を残さず、次の行へ送る本文や閉じ括弧の前に余分な空白を付けない）」「カーソルの手前がインデントだけなら行ごと下へ送る（空白だけの行を含む。元の行は空になる）」だけで、言語ごとの規則（Python の `:` 等）は持たない — 足し始めるとキリが無く、外した時に打ち直す手間の方が大きい。ペーストは素通し（貼ったテキストへインデントを足さない）。前後の空白の後始末を含めて 1 回の差し替え（`EditBuffer::replace`）にするので undo 1 回で改行前へ戻る
  - 1 段の幅は**カーソル行の前後 200 行だけ**から推定する（文書全体を舐めると 1 打鍵がファイルの大きさに比例する）。空白インデントの行が優勢なら「次の行で深くなった幅」の最頻値、タブが優勢ならタブ、材料が無ければ空白 4。**幅 1 の増加は数えない** — ブロックコメントの ` * ` や引数の桁揃えで普通に出て、1 段が空白 1 つと誤推定されるため
  - Tab は 1 文字ずつ `insert_typed` に通す（続けて打った文字と undo 1 単位にまとめるため）

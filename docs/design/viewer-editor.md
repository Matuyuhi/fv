...
- **Enter は自動インデント、Tab はソフトタブ**（`component/editor/indent.rs`、word.rs と同じく純関数）。規則は「前の行のインデントを引き継ぐ」「開き括弧 `{ ( [` の直後で 1 段下げ、直後が対応する閉じ括弧なら 3 行に開く」「インデントしか無い行で押したらその行の空白を消す」の 3 つだけで、言語ごとの規則（Python の `:` 等）は持たない — 足し始めるとキリが無く、外した時に打ち直す手間の方が大きい。ペーストは素通し（貼ったテキストへインデントを足さない）。空白行の後始末を含めて 1 回の差し替え（`EditBuffer::replace`）にするので undo 1 回で改行前へ戻る

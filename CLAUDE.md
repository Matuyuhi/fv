...
- **一覧を信用する根拠は FS 監視**。`App::on_tick` が毎 tick `FsWatcher::is_active` を `set_watched` で伝え、走査を起こした時点と完走した時点の両方で監視が生きていた時だけ `trusted` が立つ。構造の変化（作成・削除・リネーム、inotify のキュー溢れ = `Event::need_rescan` も root 全体の構造変化として通す）は `invalidate` で一覧を捨てる側へ、内容だけの変化（`Modify(Data)`・fv 上の保存）は `touch(path)` でその項目だけ `dirty` にして一覧は使い続ける（AI が 1 ファイルを書き換え続ける状況で歩き直さないため）。一覧に無いパスの `touch`・走査中に来た通知・監視の途切れ・走査条件の切替はいずれも信用しない側に倒す。**監視が無い環境では毎回 walk 経路**（stat で照合するので結果は常に正しく、read だけ省ける）
...
- **使い続けても膨らまない境界**: cache の本文は `MAX_CACHE_BYTES` で頭打ち、項目数は完走のたびに「今あるファイル」だけに作り直される（消えたファイルが残るのはキャンセルされた走査の間だけ）。snapshot の一覧は cache と同じ `Arc<Entry>` を指すので二重には持たず、`touch` で dirty にした項目は古い本文を手放す（`Entry::without_content`。読み直した版だけが cache に残る）。キャンセルされた走査の cache 反映は読み直した項目が 1 つでもある時だけ map を複製する（打鍵のたびに 10 万項目を複製しない）

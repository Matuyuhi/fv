...
- **座標は VIEW の `/` と同じ規則**: smart-case、列は plain（タブ展開済み）の char index。ファイル全体を**バイト列のまま**流して（`find_at`: 先頭バイトの候補を 8 バイト SWAR の `find_byte2` で拾い、候補だけ `eq_ignore_ascii_case` で突き合わせる）、当たった行だけ `from_utf8_lossy` → `text::normalize` → `viewer::line_matches`（`search_matches` の行単位の中身。1 ファイル 200 件の残り枠で `take` する）にかけ直すので、`Enter` で開いた先は `Viewer::locate_search` が**同じクエリで `/` を立て直すだけ**で同じ位置（行 + 桁）が光り、n/N が続けて効く。**ファイルの大きさに比例する仕事はバイト走査 1 回だけ** — UTF-8 検証・小文字化の写し・`Vec<char>` 化は当たった行に限る（stat もせず `take(上限+1)` で読む。ワーカーごとのバッファを使い回す）。タブ → 空白 4 の写しだけは避けられない時があるが、それも「クエリに空白があり、ファイルにタブがある」時に限る（空白の無いクエリは展開で一致が増減しない）。速度は `cargo test --release -- --ignored grep_bench --nocapture` で見る

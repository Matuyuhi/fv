...
- `component/` — 状態 + その状態だけを受け取る View。各フォルダは `mod.rs`（状態）と `view.rs`（描画）を持つ
  ...
  - `viewer/` — mod.rs(open/reload/履歴・cache), viewport.rs(Viewport: スクロール・折返し状態), highlight.rs(Highlighter: syntect 一式とテーマ + 行単位で再開できる Session/LineState), render.rs(HighlightCache: 可視範囲だけを組み立てる遅延ハイライト), content.rs(読込・Content/TextDoc/Open), search.rs, view.rs
...
- **遅くなっていても PR は落とさない**。UI スナップショット差分と同じ方針で、閾値（20%）を超えた時だけ `::warning::` を出す。base にこの計測が無い場合（これを追加した PR 等）は head の値だけ出す

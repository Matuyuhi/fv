...
- `component/` — 状態 + その状態だけを受け取る View。各フォルダは `mod.rs`（状態）と `view.rs`（描画）を持つ
  ...
  - `gitlane/` — GIT レーンの diff 表示状態。mod.rs が GitState (今どの diff をどう見ているか・行カーソル・行単位選択) と定数/Kind/各 *Diff 構造体、render.rs が inline の行組み立て (render_inline / LOG レーン・PR タブと共有する render_commit)、side.rs が side-by-side (#30)、word.rs が word-level 差分の範囲計算 (#29)、patch.rs が行単位 stage のパッチ組み立て、view.rs が描画
...
- **diff ペインの行カーソル**（`GitState::cursor`）。閲覧レーンと違い diff ペインには「今どこを見ているか」を示すものが枠の色しか無く、`Space` の対象も暗黙だった（上記の hunk 序数バグの原因でもある）。`j`/`k`・`Ctrl+d`/`u`・`gg`/`G` はスクロールではなく**カーソル**を動かし、画面はそれに追従する（`ensure_cursor_visible`）。`]`/`[`・`}`/`{`・検索ジャンプもカーソルを連れて動き、ホイールのスクロール（`scroll_by`）は逆にカーソルを画面内へ引き戻す — 対象が画面外に居るまま押せる状態を作らないため。クリック（`App::click_diff_row` → `GitState::click_row`）でも動かせる
  ...
  - **追従の下端は視覚行数に依存する**ので、wrap 中は `min_scroll_for_cursor` がカーソルから上へ `text::wrap_rows` で遡って最小の scroll を出す（O(画面行数)）。非 wrap と side-by-side（描画側が事前に行数を揃える）は視覚行 = 論理行なので単純な引き算で済む
  ...
  - **`GitState::new` は右ペインの実測サイズを受け取る**。diff ペインは VIEW/EDIT と同じ Rect を使うので、GIT に入った直後（まだ `draw_git` の書き戻しが無い時点）の 1 打鍵でカーソル追従が暴れないよう初期値を引き継ぐ

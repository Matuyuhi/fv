...
- `component/` — 状態 + その状態だけを受け取る View。各フォルダは `mod.rs`（状態）と `view.rs`（描画）を持つ
  ...
  - `gitlane/` — GIT レーンの diff 表示状態。mod.rs が GitState (今どの diff をどう見ているか・行カーソル) と定数/Kind/各 *Diff 構造体、render.rs が inline の行組み立て (render_inline / LOG レーン・PR タブと共有する render_commit)、side.rs が side-by-side (#30)、word.rs が word-level 差分の範囲計算 (#29)、patch.rs が行単位 stage/unstage のパッチ組み立て (`git add -p` と同じ書き換え規則。生 diff だけを見る純粋関数なのでユニットテストを直に置ける)、view.rs が描画
...
GIT レーン右ペインの `Space`（hunk 単位ステージ）と `S`（行単位ステージ）は `on_git_key` の中で、`A`/`t` と同じく `Lane::Git` の可変借用より前で拾う（git の実行と `rescan_now` に `&mut self` が要るため）。ツリー側（Focus::Tree）の `Space` はファイル単位のトグルなので、同じ「index へ移す」操作が **ファイル → hunk → 行** の 3 段階の粒度で並ぶ（粒度はフォーカスとキーで決まる）。`V`（行選択のトグル）・`Esc`（解除）は diff ペインの中で閉じる操作なので、VIEW の範囲選択キー（v/y/Y/Esc）と同じくグローバルへは持ち上げず `on_git_key` の中に置く。
...
- **右ペインは行カーソル（`GitState::cursor`）を持つ**。当初は Space の対象を「上端に見えている行が属する hunk」と定義していたが、**その行が画面のどこなのかを示すものが何も無く**（diff ペインには EDIT のような文字カーソルが無い）、j/k がスクロールしかしないため「読んでいる hunk」と「Space が効く hunk」が常時食い違っていた。`j`/`k`/`Ctrl+d`/`Ctrl+u`/`gg`/`G`・`]`/`[`・`}`/`{`・検索ジャンプ・クリックはすべてこのカーソルを動かし、viewport の方が追従する（`ensure_cursor_visible`）。マウスホイール（`scroll_by`）だけは逆に画面を動かしてカーソルを可視範囲の端へ寄せる — カーソルが画面外に置き去りになると、見えていない行に対して Space/S を押せてしまうため
  - 描画は `TextPane::cursor_band`（閉区間）で **行の帯**として出す。文字単位のカーソル（`TextPane::cursor` の REVERSED 重ね）を使わないのは、diff の行には桁の意味が無く「何桁目に居るか」を持つ必要がないため。帯は**背景色が未設定の span にだけ**敷くので、word-level ハイライト（#29）の濃い背景は帯の中でもそのまま残る。span 数・char 数を変えないので桁インバリアントにも影響しない。右端をペイン幅まで伸ばす pad だけは hscroll/wrap の**後**（`fill_row`）に置く — 行なりのラギッドな右端だと「今どの行に居るか」が行の長さで揺れて読み取りにくいが、桁計算より前に足すと VIEW の範囲選択で見送ったのと同じ「折返し位置・hscroll の桁がずれる」問題を踏むため
  - `viewport.height` が 0 の間（そのレーンを 1 度も描いていない）は `ensure_cursor_visible` を no-op にする。height を 1 と見なすと、描画前に届いたキーでカーソル行が画面上端に貼り付いてしまう（プレビューの「描画 → setup → 描画」で実際に踏んだ）
...
- **右ペイン（Focus::Viewer）の `Space` は hunk 単位の stage/unstage**（`App::stage_current_hunk`）。ツリー側の Space と同じキーに粒度違いを割り当てているのは、「ファイルを選んで Space」「diff を読みながら Space」がフォーカスで自然に切り替わるため。#20 で「実装が重いので後回し」とされていた項目で、`git apply --cached` を使う
  - **対象は「行カーソルが属する hunk」**（`GitState::current_hunk_ordinal` = `hunks()` を `cursor_line()` で二分探索）。`]`/`[` のジャンプ先とそのまま一致する定義で、ペインタイトルにも `hunk k/n` を常に出して暗黙にしない
...
- **右ペインの `S` は行単位の stage/unstage**（`App::stage_current_lines` + `component/gitlane/patch.rs`）。対象はカーソル行、`V` で行選択を始めていればその範囲（`j`/`k` が伸縮に化ける。VIEW の `v` と同じ作法で、`Esc` で解除）。hunk 単位と同じく `Mode::Confirm` を挟まず、同じ `STAGE_DEBOUNCE`・同じ `rescan_now` に相乗りする（粒度だけが違う操作なので分岐を増やさない）
  - **表示行 → 生 diff の写像は `GitDiff::body_raw`**（inline 行 index → raw の行 index）。`classify` が落とすのはファイルヘッダ（最初の `@@` より手前）だけで、それ以降は 1:1 かつ単調増加なので、**表示行の閉区間はそのまま生 diff 上の閉区間 `[lo, hi]` に写せる**。集合を持ち回らず 2 つの index だけで済むのはこの連続性が根拠
  - **選ばなかった変更行の扱いが `git add -p` の行選択そのもの**（`patch::transform_hunk`）: stage（順方向 apply）は「選ばなかった `+` を落とし、選ばなかった `-` を文脈行にする」／unstage（`--reverse` apply）は pre-image が新側（index）になるので**向きが反転**し「選ばなかった `+` を文脈行にし、選ばなかった `-` を落とす」。`\ No newline at end of file` は直前の行に紐づく注記なので、その行を落としたら一緒に落とす
  - **hunk header の開始行番号は書き換えず、行数だけ数え直す**。開始行は hunk 単位と同じ理由（`git apply` が文脈行を照合するのでオフセットを吸収する）で触らないが、**行数は書き換えた本文と食い違うと git がパッチを弾く**ので必ず数え直す。選択された変更行を 1 つも含まない hunk はパッチから丸ごと落とす
  - **inline 表示限定**（`LinePatch::SideBySide` / `ShowingAll` で拒否）。side-by-side は 1 つの行 index が左右 2 本の行を指すので生 diff に写せず、まとめ diff (`A`) はファイルヘッダを 1 つに決められない（`/` 検索を side-by-side で出さないのと同じ理由）。untracked の拒否は hunk 単位と同じ `GitDiff::stageable` を共有する

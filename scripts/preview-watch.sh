...
#   cargo watch -x 'run --quiet -- --preview git --color'
...
while :; do
    ...
    if [ "$current" != "$previous" ]; then
        previous="$current"
        clear
        printf '\033[2m$ cargo run --quiet -- --preview %s\033[0m\n' "$scenes"
        # shellcheck disable=SC2086 # scenes は複数シーン名として分割したい
        cargo run --quiet -- --preview $scenes --color || true
    fi
    sleep 1
done

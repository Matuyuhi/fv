...
mod tests {
    ...
    /// 全角文字は 2 桁を占める。buffer_lines が覆われたセルを読み飛ばすのと
    /// map_columns の数え方が食い違うと、マスクの位置決め (snapshot.rs) が桁でずれる
    #[test]
    fn wide_cells_count_as_two_columns() {
        let mut buffer = Buffer::empty(Rect::new(0, 0, 10, 1));
        buffer.set_string(0, 0, "\u{3042}a", Style::default().fg(Color::Green));
        let text = buffer_lines(&buffer, false);
        assert_eq!(text[0].chars().count(), 9);

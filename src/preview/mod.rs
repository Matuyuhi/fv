...
/// 呼び出し側が同じ 1 フレームを文字 (`render::buffer_lines`) にも画像 (`svg::render`) にも
/// 落とし、スナップショット時にはセルを直接書き換える (`snapshot::mask`) ため。
/// 「描画 → setup → 描画」と 2 回描くのは、viewport の高さ・幅やペインの Rect を ui が
/// App へ書き戻す構造に合わせるため。1 回目で実測値が入り、setup のキー列 (Ctrl+d のような
/// height 依存の操作) が実際のアプリと同じ値を見る

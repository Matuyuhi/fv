...
//! 測った 2 点の比」だけ。
//!
//! 対象を 2 つの合成物に分けているのは、git repo にするとファイルを開くところで `git diff`
//! の実行時間が混ざり、素の描画コストが読めなくなるため。build_fixture が VIEW/EDIT の
//! 素の描画コストで、build_git_fixture が EDIT のライブ diff と GIT レーンの diff ペイン。

...
pub(crate) fn short_date(updated_at: &str) -> &str {
...
// PR タブ (component/prs/view.rs) のタイトルハイライトとも共有する (positions は char インデックス)
...
// issues.lines() の中に埋め込まれている (issues::build_detail_display 参照)。TextPane に
// 一本化されている既存の描画パイプラインをそのまま使う (gutter_width は 0、search/cursor は使わない)
pub(crate) fn draw_issues_detail(
    frame: &mut Frame,
    issues: &mut IssuesState,
    focused: bool,
    background: Color,
    ...
    draw_text_detail(
        frame,
        title,
        t(Msg::IssuesEnterLClickOpenDetail),
        false,
        None,
        issues.lines(),
        &issues.viewport,

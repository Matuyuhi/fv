//! issues / pull requests タブが共有する描画部品。「読み込み中/エラー/0 件/一覧」
//! の分岐と List ウィジェットの組み立て、右ペインのプレーンテキスト表示 (issues の詳細・PR の
//! 説明/CI ステータス) はどちらも同じ形なので、行の型ごとの違いはクロージャに追い出して
//! 1 度だけ実装する。
...
pub(crate) fn draw_remote_list<R>(
    frame: &mut Frame,
    title: String,

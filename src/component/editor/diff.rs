...
// word-level diff で 1 行に許す char 数上限。行の対応付け自体は gitlane 側で
...
///
/// これを持ち回すのは、1 打鍵ごとに文書全体を舐め直さないため。編集は 1 行に閉じることが
/// 多く、**触っていない行の一致・不一致は変わらない**ので、前回の結果から次の下限が O(1)
/// で出せる。走査はその下限の続きから伸ばすだけなので、求まる値は毎回 0 から数えたのと同じになる
...
impl CommonTrim {
    ...
    /// 共通範囲の**外側**を触った場合は一致に転じて範囲が伸びうるが、その伸びは
    /// changed_lines 側の走査が続きから拾う。

//! ハイライト済み表示行の遅延生成。
//!
//! **画面に映る範囲だけ**をその場で組み立てる。syntect のハイライトは前の行の状態に依存する
//! 逐次処理なので、任意の行から再開できるよう CHECKPOINT_STRIDE 行ごとにパーサ状態
//! (LineState) を保存しておき、可視範囲の直前にある checkpoint から助走して必要な行だけを作る。
//!
//! 組み立て済みの行は**可視範囲そのものではなく前後に余白を持つ帯 (Band)** として保持し、
//! 行ごとの「その行を解析する直前の状態」も一緒に持つ。これで `j`/`k` の 1 行スクロールは
//! 帯の端に 1 行足すだけで済み、1 文字のタイピングは変更行を作り直してパーサ状態が元へ
//! 戻った時点で打ち切れる。詳細は CLAUDE.md「閲覧と編集の関係」。

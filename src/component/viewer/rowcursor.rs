...
//!
//! GIT/LOG/PR の diff と VIEW・EDIT の全ペインが同じ噛み合わせ方 (上へはみ出したら scroll を
//! カーソルまで下げ、下へはみ出したらカーソルが最下段に来る scroll まで上げる) を必要とする
//! ので、折返し込みの視覚行計算を `text::WrapCursor` と同じ規則で 1 箇所に置く。
//!
//! カーソルの実体は各コンポーネントが持ち、ここには置かない。「何行あるか」(`line_count`)・
//! 「1 論理行の視覚行数」(`rows_at`) の求め方がペインごとに違うため、状態ではなく計算だけを
//! 共有する。全て純関数にしてあるのは借用のため (CLAUDE.md「行カーソル」)。
